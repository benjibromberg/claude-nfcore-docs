...
from tests.conftest import (
    SKILL_MD, CACHE_EXISTS, DOCS_ROOT, API_ROOT,
    extract_first_bash_block, extract_python_from_bash,
)
...
def test_index_lists_all_categories():
    output = _run_index_script()
    for category in [
        "community/", "contributing/", "developing/",
        "get_started/", "nf-core-tools/", "running/", "specifications/",
    ]:
...
def test_index_file_count():
    ...
    file_lines = [l for l in output.split("\n") if l.startswith("- ") and "`" in l]
